...
function ratesAverage(movies) {
  ...
  let totalRate = movies.reduce((sum, singleMovie)=>{
    if (singleMovie.rate){
      return sum + singleMovie.rate
    }else{
      return sum
    }
    
  }, 0)
  let avg = totalRate / movies.length

  return Number(avg.toFixed(2)); // return the average converted into a number and only two decimals
